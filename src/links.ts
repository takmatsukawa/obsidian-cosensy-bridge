...
import { filePathToLinkText, removeBlockReference, shouldExcludePath } from "./utils";
import { TwohopLink } from "./model/TwohopLink";
import { getSortFunction, getTagHierarchySortFunction, getTwoHopSortFunction } from "./sort";
...
export async function getForwardLinks(
    ...
    if (activeFileCache != null && (activeFileCache.links != null || activeFileCache.embeds != null)) {
        ...
        for (const it of linkEntities) {
            ...
            if (!seen.has(key)) {
                seen.add(key);
                const targetFile = app.metadataCache.getFirstLinkpathDest(key, activeFile.path);

                if (targetFile && shouldExcludePath(targetFile.path, settings.excludePaths)) {
                    continue;
                }
                ...
                if (targetFile) {
                    ...
                    const backlinksCount = await getBacklinksCount(it.link, activeFile.path);
    ...
    const sortedResolvedLinks = await getSortedFileEntities(resolvedLinks, (entity) => entity.sourcePath, settings.sortOrder);
    return {
...
export async function getBacklinksCount(file: string, excludeFile?: string): Promise<number> {
...
export async function getBackLinks(
    ...
    return await getSortedFileEntities(backLinkEntities, (entity) => entity.sourcePath, settings.sortOrder);
}
...
export async function getTwohopLinks(
    ...
    const twoHopLinkEntities = (await Promise.all(
        linkKeys
            ...
            .map(async (path) => {
                if (twoHopLinks[path]) {
                    const sortedFileEntities = await getSortedFileEntities(twoHopLinks[path], (entity) => {
                        const file = app.metadataCache.getFirstLinkpathDest(entity.linkText, entity.sourcePath);
...
export async function getLinksListOfFilesWithTag(
    ...
    const activeFileTags = getTagsFromCache(activeFileCache, settings.excludeTags);
    ...
    const markdownFiles = app.vault.getMarkdownFiles().filter((markdownFile: TFile) =>
        markdownFile !== activeFile && !shouldExcludePath(markdownFile.path, settings.excludePaths));

    for (const markdownFile of markdownFiles) {
        const cachedMetadata = app.metadataCache.getFileCache(markdownFile);
        if (!cachedMetadata) continue;
        const fileTags = getTagsFromCache(cachedMetadata, settings.excludePaths).sort((a: string | any[], b: string | any[]) => b.length - a.length);

        for (const tag of fileTags) {
            if (!activeFileTagSet.has(tag)) continue;

            tagMap[tag] = tagMap[tag] ?? [];
            if (settings.enableDuplicateRemoval &&
                (seen[markdownFile.path] || forwardLinkSet.has(filePathToLinkText(markdownFile.path)) ||
                    twoHopLinkSet.has(filePathToLinkText(markdownFile.path)))) continue;
...
export async function createTagLinkEntities(
    ...
    const tagLinksEntitiesPromises = Object.entries(tagMap).map(async ([tag, entities]) => {
        const sortedEntities = await getSortedFileEntities(entities, (entity) => entity.sourcePath, settings.sortOrder);
...
export function getTagsFromCache(cache: CachedMetadata | null | undefined, excludeTags: string[]): string[] {
    let tags: string[] = [];
    if (cache) {
        if (cache.tags) {
...
export async function aggregate2hopLinks(activeFile: TFile, links: Record<string, Record<string, number>>): Promise<Record<string, string[]>> {
...
export async function getSortedFileEntities(entities: FileEntity[], sourcePathFn: (entity: FileEntity) => string, sortOrder: string): Promise<FileEntity[]> {
