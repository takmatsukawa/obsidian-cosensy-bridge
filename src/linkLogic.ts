...
import { getForwardLinks, getTwohopLinks, getBackLinks, getLinksListOfFilesWithTag } from "./links";
...
export async function gatherTwoHopLinks(settings: any, activeFile: TFile | null): Promise<{
    ...
    if (activeFile) {
        ...
        const sortedFiles = await getSortedFiles(allMarkdownFiles, getSortFunctionForFile(settings.sortOrder));
